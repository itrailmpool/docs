...
- `ports` определяет настройки для stratum портов, которые пул будет слушать для подключения майнеров.
  ...
  - `tls` параметр указывает, будет ли пул использовать TLS (транспортный уровень безопасности) для шифрования связи с майнерами. В этом случае TLS отключен.
...
- `daemons` список нод блокчейна, которые пул будет использовать для подтверждения транзакций и иного взаимодействия с сетью криптовалюты.
  - `host` IP-адрес, на котором запущен демон блокчейна. В данном случае это адрес localhost, что указывает на то, что демон Bitcoin запущен на той же машине, что и пул Miningcore.
  - `port` номер порта, на котором демон Bitcoin принимает RPC-подключения.
